...
function grax_tm_hero_buttons() {
    "use strict";

    var buttons = jQuery('.fn_button_animation');

    // Asegurar que estén preparados para animar
    buttons.addClass('ready');

    // Animación escalonada de los botones
    buttons.each(function (index) {
        var element = jQuery(this);
        var delay = index * 300;  // 300 ms entre cada botón

        setTimeout(function () {
            element.addClass('opened');  // Añadir la clase que inicia la animación
        }, delay);
    });
}
...
function grax_tm_hero_title_fade() {
    ...
    setTimeout(function () {
        ...
        // Animar los botones después del título
        grax_tm_hero_buttons();
    }, a + wait);
}
...
function grax_tm_anchor(){
	...
	jQuery('.anchor_nav, .scroll_link').onePageNav();
}
...
function grax_tm_down() {
    ...
    // Añadimos también los botones del hero con .scroll_link
    jQuery('.grax_tm_talk .button a, .scroll_link').on('click', function () {
        if ($.attr(this, 'href') !== '#') {
            $('html, body').animate({
                scrollTop: $($.attr(this, 'href')).offset().top - 70  // Ajusta la posición final
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    playButton.addEventListener('click', function () {
        youtubeVideo.src = "https://www.youtube.com/embed/pJ4W3mDaeKM?si=exampleSI&autoplay=1";
    ...
    closeButton.addEventListener('click', function () {
        ...
        youtubeVideo.src = '';  // Detener el video
    });
	...
	window.addEventListener('scroll', function () {
		...
		// Obtener la posición y dimensiones de la sección
		const rect = section.getBoundingClientRect();
	
		// Solo ejecutar el efecto si la sección está visible en el viewport
		if (rect.top < window.innerHeight && rect.bottom > 0) {
			// Calcula el porcentaje de scroll dentro de la sección
			const sectionHeight = section.offsetHeight;
			const scrollOffset = (window.innerHeight - rect.top) / sectionHeight;
	
			// Aumenta el desplazamiento del parallax
			const parallaxOffset = scrollOffset * 200 - 100;  // Ajuste para un efecto más notorio
...
/****************************/ 
    ...
    document.addEventListener("DOMContentLoaded", function () {
        const buttons = document.querySelectorAll(".grax_tm_button_why");
        ...
        buttons.forEach(button => {
            button.addEventListener("click", function (event) {
                ...
                // Log para verificar si el evento se activa
                console.log(`Botón clickeado: ${button.id}`);

                // Ocultar todas las secciones
                sections.forEach(section => {
                    section.classList.add("section_animation_hidden");
                });

                // Mostrar la sección correspondiente
                const targetSection = document.getElementById(button.dataset.section);
                targetSection.classList.remove("section_animation_hidden");
